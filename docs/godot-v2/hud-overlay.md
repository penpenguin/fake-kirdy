...
- current score
- orb icon row with acquired orbs in the bright icon color and missing-orb silhouettes for remaining rewards
...
The visual contract is covered by the Godot visual snapshot suite. HUD snapshots should include the top bar frame, HP bar, ability chip, item progress, orb icon row, score chip, objective text, cooldown, and icon-like cues without cropped or overlapping text at the configured viewport.
