...
`HudOverlay.gd` is the polished in-run HUD for the Godot mainline. It keeps the same trace-friendly payload owned by `GameSession.gd`, but presents it with a compact game HUD layout instead of a plain debug label stack.

Displayed state:

- current level id
- player HP and max HP in an HP bar
- current ability type in an ability chip
- collected item progress in an item progress chip
- current score
- run outcome/status in an outcome badge
...
The visual contract is covered by the Godot visual snapshot suite. HUD snapshots should include the panel frame, HP bar, ability chip, item progress, score chip, objective text, cooldown, and status without cropped or overlapping text at the configured viewport.
