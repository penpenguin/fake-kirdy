...
- [x] 2026-05-29: Added the Godot Web export preset, made the default `build` command target `dist/index.html`, added strict `build:public` output for GitHub Pages, and removed Phaser/Vite deployment guidance.
...
- 2026-05-29: Use Godot Web export as the public deployment artifact. Local `npm run build` remains graceful for machines without export templates, while `npm run build:public` is strict so CI cannot publish stale or missing `dist/` output.
