...
- `sfx_volume_scale` preserves ability and combat cue volume relative to the user setting.
...
`update_audio_mix()` applies the current settings volume, BGM/SFX scales, and ducking state to `BgmPlayer` and `SfxPlayer`. When replay evidence is useful it emits `audio.mix.updated` with `setting_volume`, `bgm_volume`, `sfx_volume`, `ui_sfx_volume`, `ducking_active`, and the triggering reason.
