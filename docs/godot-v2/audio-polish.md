...
`GameSession.gd` routes BGM and SFX assets through one traceable mix contract.

Runtime mix controls:

- `bgm_volume_scale` keeps background music below gameplay effects.
- `sfx_volume_scale` caps combat and pickup cues below the user setting.
...
Pause, settings, focus movement, and settings changes play a short UI cue through `play_ui_sfx()`. Combat capture, swallow, spit, and ability attacks use gameplay SFX.
