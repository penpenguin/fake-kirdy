...
Godot save persistence stores progression state that replay traces can verify.
...
On Web exports, `SaveStore.gd` uses `localStorage["kirdy-save"]` as the primary browser save backend through `JavaScriptBridge`. A successful browser primary write emits `save.written` with `storage_backend: "localStorage"` and also emits `save.local_storage.written`.
...
The save schema is intentionally small and focused on replay-verifiable progression state.
